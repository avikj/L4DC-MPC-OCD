...
def experiment(
    ...
    car = LinearTargetSpeedPlannerCar(
        ...
        weights=weights_tf,
        friction=0.0,
        planner_args=dict(n_iter=20, learning_rate=5.0),
    )
    world.add_car(car)
    ...
    other_car_2 = FixedVelocityCar(world, np.array([0.1, -0.2, 0.8, np.pi / 2]),
                                   horizon=5, color='gray', opacity=0.8)
    world.add_cars([other_car_1, other_car_2])
    # END TEMP SECTION
    ...
    car.init_state =tf.constant(np.array([0.0, 0.0, 2.0, np.pi / 2]), dtype=tf.float32)
    world.reset()
    trajectory2 = []
    for t in range(traj_len):
        past_state, controls, next_state = world.step(dt=0.1)
    ...
    theta_cioc = cioc.rationalize_trajectories([trajectory, trajectory2]).numpy()
    print(theta_cioc)
