...
@ex.config
def dont_crash_cioc_beta_config():
    debug_level: int = 20
    weight_norms: List[Union[float, int]] = [10.0 ** (i / 2) for i in range(10)]
    initial_theta: np.ndarray = np.ones((1,), dtype=np.float32)
    horizon = 5
    traj_len = 15

@ex.main
def dont_crash_cioc_beta_experiment(
    debug_level: int,
    ...
    true_weights = np.array([0.06129365 , 1.18058791 , 0.09916868 ,-0.15576046 ,-0.75201867 ,-0.7258584])
