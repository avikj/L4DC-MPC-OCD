...
from interact_drive.reward_inference.bayesopt_rd import finite_horizon_env
from interact_drive.reward_inference.bord_local_opt_scenario import local_opt_env
from experiments.planner_world import setup_world as replanning_env
import numpy as np
...
def main():
	...
	parser.add_argument('optimizer', type=str, choices=['random', 'bayesopt', 'cmaes', 'vis'], 
	...
	parser.add_argument('--seed', type=int, default=1)# --root_init_seed
	...
	env_seeds = [args.seed*1000000+i for i in range(args.n_inits)]
...
def run_opt(_):
	...
	if args.optimizer == 'bayesopt':
		bord.optimize(n_iter=400, seed=optimization_seed)
	elif args.optimizer == 'random':
		bord.optimize_random_search(n_iter=400, seed=optimization_seed)
		...
		im.save(f"{args.scenario}__designer_weights_{fmt(car.weights)}__heatmap.png")
		bord.eval_weights(car.weights, gif=f'{args.scenario}__designer_weights_{fmt(car.weights)}.gif', heatmap_show=False)
		car.weights = env["tuned_weights"]
		world.reset()
		heatmap_frame = world.render(mode='rgb_array', heatmap_show=True)
		im = Image.fromarray(heatmap_frame)
		im.save(f'{args.scenario}__designer_weights_{fmt(car.weights)}__agent_weights_{env["tuned_weights"]}__heatmap.png')
		bord.eval_weights(env["tuned_weights"], gif=f'{args.scenario}__designer_weights_{fmt(car.weights)}__agent_weights_{env["tuned_weights"]}.gif', heatmap_show=False)
	else:
		assert args.optimizer == 'cmaes'
		bord.optimize_cmaes(sigma0=args.sigma, seed=optimization_seed)
